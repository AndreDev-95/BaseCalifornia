...
const streetPrimaryEl = document.getElementById('street-primary');
const streetSecondaryEl = document.getElementById('street-secondary');
...
function updateFuel(fuel = 0) {
  ...
  fuelEl.classList.add(safe <= 20 ? 'warning' : 'success');
}
...
function updateLocation(primary = '', secondary = '') {
  const line1 = (primary || '').trim();
  const line2 = (secondary || '').trim();
  const visible = Boolean(line1 || line2);

  if (streetPrimaryEl) streetPrimaryEl.textContent = line1 || '---';
  if (streetSecondaryEl) streetSecondaryEl.textContent = line2 || '---';
  setVisible(locationEl, visible);
}
...
window.addEventListener('message', (event) => {
  ...
  if (data.action === 'baseplate') {
    updateLocation(data.street1 || data.street || '', data.zone || data.street2 || '');
  }
});
